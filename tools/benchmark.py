...
def generate_synthetic_video(path: str, num_frames: int=200, width: int=1920, height: int=1080) -> None:
    ...
    try:
        ...
        for i in range(num_frames):
            ...
            cv2.putText(frame, f'Benchmark Frame {i:03d}', (80, 140), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (0, 0, 255), 4)
            writer.write(frame)
    finally:
        writer.release()
    print(f'Synthetic video generated at: {path}')

def benchmark_stages(video_path: str, cols: int=200, cell_w: int=8, cell_h: int=14, num_frames: int=200) -> dict[str, float]:
    ...
    try:
        ...
        sink = FrameSink(source_path=video_path, output_path=tmp_out_path, width=grid.out_w, height=grid.out_h, fps=info.fps, crf=18, ffmpeg_path=ffmpeg_bin)
        t0 = time.perf_counter()
        with sink:
    ...
    stages = [('1. Video Decode', t_decode), ('2. Downsample (cv2.INTER_AREA)', t_downsample), ('3. Mask Construction (atlas[digits])', t_mask), ('4. Colourise & Multiply (in-place)', t_colourise_opt), ('5. Encoder Write (FFmpeg pipe)', t_encoder)]
    ...
    print('-' * 75)
    print(f'  * Upscaling to full grid ({grid.out_w}x{grid.out_h}):\n      cv2.resize (INTER_NEAREST) : {t_upscale_nearest:.3f}s ({t_upscale_nearest / actual_frames * 1000:.2f} ms/frame)\n      np.repeat on both axes     : {t_upscale_repeat:.3f}s ({t_upscale_repeat / actual_frames * 1000:.2f} ms/frame)\n      Speedup: {t_upscale_repeat / t_upscale_nearest:.2f}x faster using cv2.INTER_NEAREST\n')
    print(f'  * Multiply & Combine buffer allocation:\n      Allocating new array/frame : {t_multiply_allocated:.3f}s ({t_multiply_allocated / actual_frames * 1000:.2f} ms/frame)\n      In-place pre-allocated buf : {t_multiply_inplace:.3f}s ({t_multiply_inplace / actual_frames * 1000:.2f} ms/frame)\n      Speedup: {t_multiply_allocated / t_multiply_inplace:.2f}x faster with pre-allocated buffer\n')
    return {'t_decode': t_decode, 't_downsample': t_downsample, 't_mask': t_mask, 't_colourise_opt': t_colourise_opt, 't_colourise_baseline': t_colourise_baseline, 't_upscale_nearest': t_upscale_nearest, 't_upscale_repeat': t_upscale_repeat, 't_multiply_allocated': t_multiply_allocated, 't_multiply_inplace': t_multiply_inplace, 't_encoder': t_encoder, 'total_time': total_time}
...
def benchmark_multiprocessing(video_path: str, num_frames: int=200, cols: int=200) -> None:
    ...
    for w in worker_counts:
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as tmp_out:
            tmp_out_path = tmp_out.name
        try:
            config = RenderConfig(source_path=video_path, output_path=tmp_out_path, cols=cols, workers=w, limit=num_frames)
            t0 = time.perf_counter()
...
def main() -> None:
    ...
    try:
        benchmark_stages(video_path=video_path, cols=args.cols, num_frames=args.frames)
        if not args.skip_mp:
            benchmark_multiprocessing(video_path=video_path, num_frames=args.frames, cols=args.cols)
    finally:
