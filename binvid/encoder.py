...
def _get_cfr_flag(ffmpeg_bin: str) -> list[str]:
    """Return ['-fps_mode', 'cfr'] or ['-vsync', 'cfr'] depending on FFmpeg version."""
    global _CFR_FLAG
    ...
    try:
        ...
        match = re.search(r"ffmpeg version (\d+)", res.stdout)
...
class FrameSink:
    """Context-manager piping raw BGR video frames into FFmpeg for H.264 video encoding.

    Captures audio from the original source_path via FFmpeg's optional audio mapping '1:a:0?'.
    Enforces constant frame rate (-fps_mode cfr / -vsync cfr) for variable-frame-rate (VFR) inputs.
    Captures stderr in a background thread to prevent pipe deadlocks and provide clear
    diagnostics on encoding failures or BrokenPipeError.
    """
    ...
    def _get_stderr_tail(self, max_lines: int = 40) -> str:
        """Return the last max_lines of FFmpeg stderr."""
        lines = list(self._stderr_buffer)[-max_lines:]
        return "".join(lines) if lines else "<No stderr captured>"

    def write(self, frame: np.ndarray) -> None:
        """Validate and write a single BGR frame into FFmpeg's rawvideo stdin pipe."""
        ...
        try:
            ...
            # Allow reader thread to capture fatal messages from FFmpeg
            if self._reader_thread and self._reader_thread.is_alive():
                self._reader_thread.join(timeout=0.5)
            stderr_tail = self._get_stderr_tail(40)
            raise RuntimeError(
                f"Broken pipe writing to FFmpeg (encoder exited prematurely):\n{stderr_tail}"
            ) from exc
    ...
    def __exit__(
        ...
        if retcode != 0:
            stderr_tail = self._get_stderr_tail(40)
            raise RuntimeError(
                f"FFmpeg encoding failed with exit code {retcode}:\n{stderr_tail}"
            )
...
def main() -> None:
    """Generate 60 frames of static at 30fps and verify output playback."""
    import shutil
    import tempfile
