...
class RenderConfig:
    ...
    def validate(self) -> None:
        ...
        if self.digit_mode not in ('static', 'refresh', 'scroll'):
            raise ValueError(f"digit_mode must be one of 'static', 'refresh', 'scroll', got '{self.digit_mode}'.")
        if self.workers < 1:
    ...
    if ',' in c:
        ...
        if len(parts) == 3:
            try:
                ...
                pass
    raise ValueError(f"Invalid background color '{color_str}'. Expected hex (#RRGGBB) or comma-separated RGB (R,G,B).")
