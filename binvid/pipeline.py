...
def _init_render_worker(grid: GridSpec, atlas: np.ndarray, mode: str, tint_color: tuple[int, int, int]=(0, 255, 70), bg_color: tuple[int, int, int]=(0, 0, 0), invert: bool=False, gamma: float=1.0, contrast_boost: bool=False, scanlines: bool=False, edge_emphasis: bool=False) -> None:
    cv2.setNumThreads(1)
    _WORKER_CTX.update({'grid': grid, 'atlas': atlas, 'mode': mode, 'tint_color': tint_color, 'bg_color': bg_color, 'invert': invert, 'gamma': gamma, 'contrast_boost': contrast_boost, 'scanlines': scanlines, 'edge_emphasis': edge_emphasis, 'out': np.empty((grid.out_h, grid.out_w, 3), dtype=np.uint8), 'temp_u16': np.empty((grid.out_h, grid.out_w, 3), dtype=np.uint16)})

def _render_worker_task(item: tuple[np.ndarray, np.ndarray]) -> np.ndarray:
    frame, digits = item
    return render_frame(bgr_frame=frame, digits=digits, **_WORKER_CTX).copy()

def _frame_task_generator(source_path: str, digit_field: DigitField | ScrollField, limit: int | None=None, downsample_size: tuple[int, int] | None=None, rotation: int=0) -> Iterator[tuple[np.ndarray, np.ndarray]]:
...
def convert(config: RenderConfig, progress: bool=True, progress_callback: Callable[[int, int | None], None] | None=None) -> None:
    config.validate()
    ...
    if config.digit_mode == 'scroll':
        ...
        digit_field = DigitField(grid.rows, grid.cols, seed=config.seed, refresh=config.digit_refresh, partial=False)
    bg_color = config.get_bg_bgr()
    tint_color = config.get_tint_bgr()
    sink = FrameSink(source_path=config.source_path, output_path=config.output_path, width=grid.out_w, height=grid.out_h, fps=info.fps, crf=config.crf, ffmpeg_path=ffmpeg_bin)
    frames_written = 0
    ...
    pbar = tqdm(total=total_frames, disable=not progress, unit='frame', desc='Rendering')
    try:
        with sink:
            if config.workers > 1:
                task_stream = _frame_task_generator(config.source_path, digit_field, config.limit, downsample_size=(grid.cols, grid.rows), rotation=info.rotation)
                with mp.Pool(processes=config.workers, initializer=_init_render_worker, initargs=(grid, atlas, config.mode, tint_color, bg_color, config.invert, config.gamma, config.contrast_boost, config.scanlines, config.edge_emphasis)) as pool:
                ...
                task_stream = _frame_task_generator(config.source_path, digit_field, config.limit, downsample_size=None, rotation=info.rotation)
                cv2.setNumThreads(1)
                ...
                for frame, digits in task_stream:
                    rendered = render_frame(frame, grid, atlas, digits, mode=config.mode, tint_color=tint_color, bg_color=bg_color, invert=config.invert, gamma=config.gamma, contrast_boost=config.contrast_boost, scanlines=config.scanlines, edge_emphasis=config.edge_emphasis, out=out_buf, temp_u16=temp_u16_buf)
                    sink.write(rendered)
                    frames_written += 1
                    pbar.update(1)
                    if progress_callback is not None:
