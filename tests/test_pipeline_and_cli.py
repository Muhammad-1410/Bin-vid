...
class TestPipelineAndCLI(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run([cls.ffmpeg_bin, '-y', '-f', 'lavfi', '-i', 'testsrc=duration=1:size=320x240:rate=30', '-f', 'lavfi', '-i', 'sine=frequency=440:duration=1', '-c:v', 'libx264', '-c:a', 'aac', cls.src_video], capture_output=True, check=True)
    ...
    def test_convert_end_to_end_color(self):
        ...
        config = RenderConfig(source_path=self.src_video, output_path=out_path, cols=40, cell_w=8, cell_h=14, mode='color', digit_mode='refresh', digit_refresh=8)
    ...
    def test_convert_end_to_end_mono_and_scroll(self):
        ...
        config = RenderConfig(source_path=self.src_video, output_path=out_path, cols=35, cell_w=8, cell_h=14, mode='mono', digit_mode='scroll')
        convert(config, progress=False)
        self.assertTrue(os.path.isfile(out_path))
        info = probe(out_path)
        self.assertEqual(info.frame_count, 30)
    ...
    def test_convert_keyboard_interrupt_handles_cleanly(self):
        ...
        config = RenderConfig(source_path=self.src_video, output_path=out_path, cols=30, cell_w=8, cell_h=14)
        call_count = 0
    ...
    def test_convert_with_limit(self):
        ...
        config = RenderConfig(source_path=self.src_video, output_path=out_path, cols=30, cell_w=8, cell_h=14, limit=12)
        convert(config, progress=False)
        self.assertTrue(os.path.isfile(out_path))
        info = probe(out_path)
    ...
    def test_convert_multiprocessing_workers(self):
        ...
        config = RenderConfig(source_path=self.src_video, output_path=out_path, cols=30, cell_w=8, cell_h=14, workers=2, limit=15)
        convert(config, progress=False)
        self.assertTrue(os.path.isfile(out_path))
        info = probe(out_path)
    ...
    def test_cli_parser_defaults_and_options(self):
        ...
        args2 = parser.parse_args(['input.mp4', '-o', 'custom.mp4', '--cols', '150', '--mode', 'mono', '--digit-mode', 'scroll', '--crf', '22', '--workers', '4', '--limit', '50'])
    ...
    def test_cli_main_execution_with_workers_and_limit(self):
        ...
        argv = [self.src_video, '-o', cli_out, '--cols', '30', '--workers', '2', '--limit', '10', '--no-progress']
        ret = cli_main(argv)
    ...
    def test_cli_main_execution(self):
        ...
        argv = [self.src_video, '-o', cli_out, '--cols', '30', '--mode', 'flat', '--no-progress']
        ret = cli_main(argv)
        self.assertEqual(ret, 0)
        self.assertTrue(os.path.isfile(cli_out))
        info = probe(cli_out)
    ...
    def test_permission_error_on_unwritable_destination(self):
        config = RenderConfig(source_path=self.src_video, output_path='/non_existent_root_dir_xyz/cannot_write/out.mp4', cols=30)
    ...
    def test_convert_with_styles(self):
        ...
        config = RenderConfig(source_path=self.src_video, output_path=out_path, cols=30, cell_w=8, cell_h=14, limit=5, invert=True, bg='#101820', gamma=0.8, contrast_boost=True, scanlines=True, edge_emphasis=True)
        convert(config, progress=False)
        self.assertTrue(os.path.isfile(out_path))
        info = probe(out_path)
