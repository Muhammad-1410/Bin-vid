...
class TestDigits(unittest.TestCase):
    ...
    def test_refresh_n_behavior(self):
        ...
        f4 = df.for_frame(4)
        # Should regenerate at frame 4
        self.assertFalse(np.array_equal(f0, f4))
        f5 = df.for_frame(5)
        self.assertIs(f4, f5)

    def test_partial_refresh_ratio(self):
        # In partial mode, ~15% of cells should flip at each refresh
        rows, cols = 100, 100
        ...
        # Expect roughly 15% flipped (tolerance: between 10% and 20%)
    ...
    def test_scrollfield_drifts_across_frames(self):
        ...
        # Scrolling should alter the visible window
        self.assertFalse(np.array_equal(f0, f5))
        self.assertFalse(np.array_equal(f5, f10))

    def test_interchangeable_interface(self):
        fields = [
            DigitField(rows=12, cols=16, seed=42, refresh=8),
            DigitField(rows=12, cols=16, seed=42, refresh=8, partial=True),
            ScrollField(rows=12, cols=16, seed=42),
        ]
        for field in fields:
