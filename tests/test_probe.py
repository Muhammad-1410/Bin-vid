...
class TestProbe(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if ffmpeg_bin:
            subprocess.run([ffmpeg_bin, '-y', '-f', 'lavfi', '-i', 'testsrc=duration=0.5:size=64x48:rate=10', '-f', 'lavfi', '-i', 'sine=frequency=1000:duration=0.5', '-c:v', 'libx264', '-c:a', 'aac', cls.video_with_audio], capture_output=True, check=False)
    ...
    def test_bad_metadata_fps_zero_falls_back(self):
        ...
        prop_map = {cv2.CAP_PROP_FRAME_WIDTH: 100.0, cv2.CAP_PROP_FRAME_HEIGHT: 50.0, cv2.CAP_PROP_FPS: 0.0, cv2.CAP_PROP_FRAME_COUNT: 60.0}
    ...
    def test_bad_metadata_frame_count_negative_falls_back(self):
        mock_cap = MagicMock()
        mock_cap.isOpened.return_value = True
        prop_map = {cv2.CAP_PROP_FRAME_WIDTH: 100.0, cv2.CAP_PROP_FRAME_HEIGHT: 50.0, cv2.CAP_PROP_FPS: 25.0, cv2.CAP_PROP_FRAME_COUNT: -1.0}
        mock_cap.get.side_effect = lambda prop: prop_map.get(prop, 0.0)
        with patch('cv2.VideoCapture', return_value=mock_cap):
            with warnings.catch_warnings(record=True) as recorded_warnings:
                warnings.simplefilter('always')
