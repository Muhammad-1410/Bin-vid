...
class TestEncoder(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run([cls.ffmpeg_bin, '-y', '-f', 'lavfi', '-i', 'testsrc=duration=1:size=160x120:rate=30', '-f', 'lavfi', '-i', 'sine=frequency=440:duration=1', '-c:v', 'libx264', '-c:a', 'aac', cls.src_with_audio], capture_output=True, check=True)
        cls.src_silent = os.path.join(cls.temp_dir, 'ref_silent.mp4')
        subprocess.run([cls.ffmpeg_bin, '-y', '-f', 'lavfi', '-i', 'testsrc=duration=1:size=160x120:rate=30', '-c:v', 'libx264', cls.src_silent], capture_output=True, check=True)
