...
def check_aspect_ratio_tolerance(src_w: int, src_h: int, out_w: int, out_h: int, max_tolerance: float = 0.02) -> None:
    ...
    assert relative_diff <= max_tolerance, (
        f"Aspect ratio discrepancy {relative_diff:.4f} ({relative_diff * 100:.2f}%) "
        f"exceeds tolerance {max_tolerance * 100:.1f}%. "
        f"src={src_w}x{src_h} (AR={src_ar:.4f}), out={out_w}x{out_h} (AR={out_ar:.4f})"
    )


class TestGeometry(unittest.TestCase):
    def test_16_9_input(self):
        # Landscape 16:9 input (1920x1080)
    ...
    def test_even_dimensions_with_odd_cells_and_odd_cols(self):
        # Stress test with odd cell sizes and odd column counts
        odd_test_cases = [
            (1920, 1080, 101, 7, 14),
            (1920, 1080, 101, 8, 14),
            (1080, 1920, 99, 7, 14),
    ...
    def test_fit_cols_to_width(self):
        # 16:9 input targeting roughly 1080p (1920 wide)
        ...
        # 16:9 input targeting 720p (1280 wide)
        ...
        # 9:16 vertical input targeting 1080 width
...
# Standalone pytest functions for direct pytest invocation
